...
class GSTSummaryPopUpSales extends StatefulWidget {
  final List<SalesEntryWithEntries> salesEntriesWithEntries;

  const GSTSummaryPopUpSales(
      {super.key, required this.salesEntriesWithEntries});

  @override
  State<GSTSummaryPopUpSales> createState() => _ChequeReturnEntryState();
}

class _ChequeReturnEntryState extends State<GSTSummaryPopUpSales> {
  ...
  void calculateTotalNetAmount() {
    for (var salesEntryWithEntries in widget.salesEntriesWithEntries) {
      for (var entry in salesEntryWithEntries.entries) {
                        ...
                        SingleChildScrollView(
                          child: SizedBox(
                            ...
                            child: ListView.builder(
                              shrinkWrap: true,
                              itemCount: widget.salesEntriesWithEntries.length,
                              itemBuilder: (context, index) {
                                final salesEntryWithEntries =
                                    widget.salesEntriesWithEntries[index];
                                String ledgerName = getLedgerName(
                                    salesEntryWithEntries.salesEntry.party);
                                ...
                                return InkWell(
                                  ...
                                  child: Table(
                                    ...
                                    children: List<TableRow>.from(
                                      salesEntryWithEntries.entries.map(
                                        (entry) {
                                          return TableRow(
                                            children: [
                                              _buildTableCell(
                                                  salesEntryWithEntries
                                                      .salesEntry.date),
                                              _buildTableCell(
                                                  salesEntryWithEntries
                                                      .salesEntry.place),
